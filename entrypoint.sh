...
echo "Waiting for RTSP cam_in stream from OBS WHIP..."
echo
...
echo "Starting DLC bridge..."
python3.11 /app/bridge.py &
BRIDGE_PID=$!

shutdown() {
  echo "Stopping..."
  kill "$BRIDGE_PID" "$MEDIAMTX_PID" 2>/dev/null || true
  wait "$BRIDGE_PID" "$MEDIAMTX_PID" 2>/dev/null || true
}
